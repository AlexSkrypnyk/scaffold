...
Diagrams are `.puml` sources in `docs/architecture/`, rendered to committed light `.svg` files that `README.md` embeds as images. Put the traced-from annotation in a `'` comment at the top of each `.puml` file.
...
Render with `plantuml -tsvg docs/architecture/*.puml`. Commit both the `.puml` sources and the generated `.svg` files.
